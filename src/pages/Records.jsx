...
// Changed primary color to green per request
...
const Records = () => {
  ...
  // tint header notification bell to START_BLUE (green) when records page mounts
  useEffect(() => {
    const selectors = [
    ...
    for (const img of imgs) {
      if ((img.src || "").toLowerCase().includes("notification") || (img.alt || "").toLowerCase().includes("notif")) {
        img.style.filter = "brightness(0) saturate(100%) sepia(0.4) hue-rotate(120deg) saturate(420%)";
        break;
      }
    }
  }, []);
